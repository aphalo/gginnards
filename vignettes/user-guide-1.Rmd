...
The structure of bjects of classes `"gg"` `"ggplot"` can be explored with R's 
...
the subset of data correspoding to a single group based on the mapping.
...
This can make it tedious to analyze how these functions work, as one may need to
...
their `data` input by default to the console, but the function used to dispaly
the data can be passed as an argument, adding flexibility. The debug stats, in
addition return a data frame containing labels suitable for plotting with geom
"text" or geom "label". However, as default geom "null" is used, they do not
alter the plot itself unless a different geom is explicitly passed as argument.
...
The 'gginnards' package also defines a `"null"` geom, which is used as default by the _debug_ _stats_ described above. Currently this geom is similar to the recently added `ggplot2::geom_blank()`.
